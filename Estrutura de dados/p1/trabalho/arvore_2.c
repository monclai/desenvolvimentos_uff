		...
		if(x==a->info){
			if(a->esq==NULL && a->dir==NULL){
				...
				while(t->dir!=NULL)
					t=t->dir;
				a->info=t->info;
				a->esq=remove(a->esq, t->info);
				return a;
			}
		}
		else{
		if(x<a->info)
			a->esq=remove(a->esq, x);
		else
			a->dir=remove(a->dir, x);
		}
	}
	return a;	
}*/
...
arvore *balancear(arvore *a){
	if(a!=NULL){
		...
		hd=altura(a->dir);
		if(he-hd>1){
			//desbalancear para esquerda	
			arvore *t;
			int x, r;
			//achar maior elemento da esquerda
			t=a->esq;
			while(t->dir!=NULL)
				t=t->dir;
			x=t->info;
			//remover o maior da esquerda
			a->esq=remover(a->esq, x);
			//substituir a raiz pelo nó removido
			r=a->info;
			a->info=x;
			//inserir r na direita
			a->dir=inserir(a->dir, r);
			a=balancear(a);			
		}
		else if(hd-he>1){
			//desbalancear para direita
			arvore *t;
			int x, r;
			//achar menor elemento da direita
			t=a->dir;
			while(t->esq!=NULL)
				t=t->esq;
			x=t->info;
			//remover o menor da direita
			a->dir=remover(a->dir, x);
			//substituir a raiz pelo nó removido
			r=a->info;
			a->info=x;
			//inserir r na esquerda
			a->esq=inserir(a->esq, r);
			a=balancear(a);				
		}
	}
	return a;
